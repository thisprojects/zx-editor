...
export function exportASM({ pixels, attributes, bounds, fileName }: ExportParams): boolean {
  ...
  lines.push('; Format: FBPPPIII where F=flash, B=bright, P=paper(0-7), I=ink(0-7)');
  lines.push('; -----------------------------------------------------------------------------');
...
export function exportTileASM({ tileSize, pixels, attributes, fileName }: TileExportParams): boolean {
  ...
  lines.push('        ld ix,tile_attr_data');
  lines.push('        ld d,TILE_HEIGHT');
  ...
  lines.push('        add hl,hl');
  lines.push('        add hl,hl');
  lines.push('        add hl,hl               ; hl = row*32');
  lines.push('        ld de,22528');
  lines.push('        add hl,de               ; hl = 22528 + row*32');
  lines.push('        ld e,c');
  lines.push('        ld d,0');
  lines.push('        add hl,de               ; hl = 22528 + row*32 + col');
  lines.push('        ex de,hl                ; de = attribute address');
  lines.push('        ld b,TILE_WIDTH');
